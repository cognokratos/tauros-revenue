defmodule TaurosWeb.Layouts do
  ...
  def app(assigns) do
    ...
    <div class="min-h-full">
      <nav aria-label="Main navigation" class="border-b border-gray-200 bg-white dark:border-white/10 dark:bg-gray-900">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div class="flex h-16 justify-between">
              ...
              <%= if @current_scope do %>
                <div class="relative group">
                  ...
                  <div class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg outline outline-black/5 dark:bg-gray-800 dark:shadow-none dark:-outline-offset-1 dark:outline-white/10 opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible transition-all duration-200">
                    ...
                    <.link href={~p"/users/settings"} class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 border-b border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-white/5">Settings</.link>
                    <.link href={~p"/users/log-out"} method="delete" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/5">Log out</.link>
                  </div>
                </div>
              <% else %>
                <.link href={~p"/users/register"} class="text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">Register</.link>
                <.link href={~p"/users/log-in"} class="text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">Log in</.link>
              <% end %>
            </div>
            ...
            <div class="-mr-2 flex items-center sm:hidden">
              <button
                type="button"
                phx-click={JS.toggle(to: "#mobile-menu", in: {"transition ease-out duration-200","opacity-0 scale-95","opacity-100 scale-100"}, out: {"transition ease-in duration-150","opacity-100 scale-100","opacity-0 scale-95"})}
                aria-expanded="false"
      ...
      <div id="mobile-menu" class="hidden sm:hidden fixed inset-x-0 top-16 z-40 w-full bg-white dark:bg-gray-800/95 backdrop-blur-sm shadow-lg max-h-[calc(100vh-4rem)] overflow-auto">
        ...
        <div class="border-t border-gray-200 pt-4 pb-3 dark:border-gray-700">
          <div class="flex items-center px-4">
            <div class="shrink-0">
              <div class="size-10 rounded-full outline -outline-offset-1 outline-black/5 dark:outline-white/10 bg-indigo-600 flex items-center justify-center text-white text-sm font-medium">{initials(@current_scope && @current_scope.user.email || "")}</div>
            </div>
            <div class="ml-3">
              <div class="text-base font-medium text-gray-800 dark:text-white">{(@current_scope && @current_scope.user.email) || ""}</div>
            </div>
          </div>

          <div class="mt-3 space-y-1 px-2">
            <.link href={~p"/users/settings"} class="block px-4 py-2 text-base font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-800">Settings</.link>
            <.link href={~p"/users/log-out"} method="delete" class="block px-4 py-2 text-base font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-800">Log out</.link>
          </div>
        </div>
      </div>
